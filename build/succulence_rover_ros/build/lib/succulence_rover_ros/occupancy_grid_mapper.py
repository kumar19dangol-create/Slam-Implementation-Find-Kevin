"""
Bayesian Occupancy Grid Mapping (Week 5 — provided complete)
...
the motivation for scan matching (Week 6) and SLAM (Week 7).

This code is PROVIDED COMPLETE — you do not need to modify it for Week 5.
...
class OccupancyGrid:
    ...
    def __init__(self,
                 ...
                 max_range: float = 3.5,
                 min_range: float = 0.1,
                 lidar_x_offset: float = 0.0,
                 lidar_y_offset: float = 0.0,
...
class OccupancyGridMapperNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('map_topic', '/succulence/map/odom_only')
        self.declare_parameter('map_publish_rate', 1.0)

        self.declare_parameter('occupancy_grid.resolution', 0.05)
        self.declare_parameter('occupancy_grid.width', 200)
        self.declare_parameter('occupancy_grid.height', 200)
        self.declare_parameter('occupancy_grid.origin_x', -5.0)
        self.declare_parameter('occupancy_grid.origin_y', -5.0)
        self.declare_parameter('occupancy_grid.log_odds_occupied', 0.85)
        self.declare_parameter('occupancy_grid.log_odds_free', 0.4)
        self.declare_parameter('occupancy_grid.log_odds_max', 5.0)
        self.declare_parameter('occupancy_grid.log_odds_min', -5.0)
        self.declare_parameter('occupancy_grid.max_range', 3.5)
        self.declare_parameter('occupancy_grid.min_range', 0.1)

        # Lidar mounting offset relative to base_link
        # Set these to match your robot's base_link → lidar TF
        # Default (0, 0, 0) works for both the Unity sim and TurtleBot
